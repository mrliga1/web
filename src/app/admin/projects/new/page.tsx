...
export default function NewProjectPage() {
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const projectsRef = dbRef(firebaseDatabase, "/projects");
  ...
  return (
    <div className="mx-auto max-w-4xl text-[#efdfa6]">
      <div className="mb-6 rounded-[2rem] border border-[#efdfa6] bg-[#05080f] p-6 shadow-[0_30px_90px_rgba(0,0,0,0.35)]">
        <h1 className="text-2xl font-semibold text-white">Tạo dự án mới</h1>
        <p className="mt-2 text-sm text-[#b9b3a1]">Thiết lập dự án mới với nội dung, vị trí và ảnh đại diện sang trọng.</p>
      </div>
      ...
      <div className="space-y-6 rounded-[2rem] bg-[#05080f] p-6 shadow-[0_30px_90px_rgba(0,0,0,0.35)] border border-[#efdfa6]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input value={location} onChange={(e) => setLocation(e.target.value)} className="mt-2 w-full rounded-3xl border border-[#efdfa6] bg-[#0d1523] px-4 py-3 text-[#efdfa6] outline-none focus:border-[#efdfa6]" />
          </div>
          <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <a href="/admin/projects" className="text-sm text-[#b9b3a1] hover:text-white">Hủy</a>
        </div>
      </div>
